...
corr_data <- barca_6y %>%
  select(id, vars_int[c(1, 5, 9)], vars_int_wo) %>%
...
var_int3 <- str_c(rep(c("viol", "prop_vh", "prop_pers"), each = 3),
                  rep(c(".f", ".p", ".r"), 3))

b6yw2_uw <- barca_6y %>%
  rename_all(~str_remove_all(., "erty|onal|ence")) %>%
  select(all_of(var_int3)) %>%
  mutate_all(~log(. + 1))
...
model <- c("t_viol =~ 1*viol.f + 1*viol.p + 1*viol.r
            t_vh =~ 1*prop_vh.f + 1*prop_vh.p + 1*prop_vh.r
            t_pers =~ 1*prop_pers.f + 1*prop_pers.p + 1*prop_pers.r

            m_rep =~ 1*viol.p + 1*prop_vh.p + 1*prop_pers.p
            m_vic =~ 1*viol.r + 1*prop_vh.r + 1*prop_pers.r
            m_loc =~ 1*viol.f + 1*prop_vh.f + 1*prop_pers.f
            ...
            viol.p + prop_vh.p + prop_pers.p + viol.r + prop_vh.r + prop_pers.r + viol.f + prop_vh.f + prop_pers.f ~ 0*1

           ")
mtmm_uw <- bsem(model, data = b6yw2_uw,
             burnin = 8000, sample = 2000, n.chains = 8)

summary(mtmm_uw, standardized = TRUE)

save(mtmm_uw, file = "./output/mtmm_uw.RData")

load("./output/mtmm_uw.RData")

summary(mtmm_uw, standardized = T)


mtmm_uw_est <- lavaan::parameterestimates(mtmm_uw, standardized = T)

mtmm_uw_qual <- mtmm_uw_est %>%
  filter(op == "=~" | op == "~~") %>%
  filter(!(op == "~~" & str_detect(lhs, "t_|m_"))) %>%
  mutate(Trait = case_when(str_detect(rhs, "viol") ~ "Violence",
                           str_detect(rhs, "vh") ~ "Property household/vehicle",
...
ggsave("./output/figs/mtmm_qual_overall_uw.png", height = 8)
...
ggsave("./output/figs/mtmm_mean_overall_uw.png")
...
b2yw_uw <- barca_2y %>%
  rename_all(~str_remove_all(., "erty|onal|ence")) %>%
  select(id, year, var_int3) %>%
  mutate(year = str_remove_all(year, "20|-.+")) %>%
  pivot_wider(
    values_from = viol.f:prop_pers.r,
    names_sep = "_",
    names_from = year) %>%
  mutate_at(vars(-id),
            ~log(. + 1))
...
# property vehicle
model <- c("t1 =~ 1*prop_vh.f_15 + 1*prop_vh.p_15 + 1*prop_vh.r_15
            t2 =~ 1*prop_vh.f_17 + 1*prop_vh.p_17 + 1*prop_vh.r_17
            t3 =~ 1*prop_vh.f_19 + 1*prop_vh.p_19 + 1*prop_vh.r_19

            t2 ~ t1
            t3 ~ t2

            m_rep =~ 1*prop_vh.p_15 + 1*prop_vh.p_17 + 1*prop_vh.p_19
            m_vic =~ 1*prop_vh.r_15 + 1*prop_vh.r_17 + 1*prop_vh.r_19
            m_loc =~ 1*prop_vh.f_15 + 1*prop_vh.f_17 + 1*prop_vh.f_19
            ...
            prop_vh.p_15 + prop_vh.p_17 + prop_vh.p_19 + prop_vh.r_15 + prop_vh.r_17 + prop_vh.r_19 + prop_vh.f_15 + prop_vh.f_17 + prop_vh.f_19 ~ 0*1

           ")


m2_propvh_uw <- bsem(model, data = b2yw_uw,
                  burnin = 8000, sample = 2000, n.chains = 8)

summary(m2_propvh_uw, standardized = TRUE)



# property personal
model <- c("t1 =~ 1*prop_pers.f_15 + 1*prop_pers.p_15 + 1*prop_pers.r_15
            t2 =~ 1*prop_pers.f_17 + 1*prop_pers.p_17 + 1*prop_pers.r_17
            t3 =~ 1*prop_pers.f_19 + 1*prop_pers.p_19 + 1*prop_pers.r_19

            t2 ~ t1
            t3 ~ t2

            m_rep =~ 1*prop_pers.p_15 + 1*prop_pers.p_17 + 1*prop_pers.p_19
            m_vic =~ 1*prop_pers.r_15 + 1*prop_pers.r_17 + 1*prop_pers.r_19
            m_loc =~ 1*prop_pers.f_15 + 1*prop_pers.f_17 + 1*prop_pers.f_19
            ...
            prop_pers.p_15 + prop_pers.p_17 + prop_pers.p_19 + prop_pers.r_15 + prop_pers.r_17 + prop_pers.r_19 + prop_pers.f_15 + prop_pers.f_17 + prop_pers.f_19 ~ 0*1

           ")
m2_proppers_uw <- bsem(model, data = b2yw_uw,
                    burnin = 8000, sample = 2000, n.chains = 8)

summary(m2_proppers_uw, standardized = TRUE)
...
# violence
model <- c("t1 =~ 1*viol.f_15 + 1*viol.p_15 + 1*viol.r_15
            t2 =~ 1*viol.f_17 + 1*viol.p_17 + 1*viol.r_17
            t3 =~ 1*viol.f_19 + 1*viol.p_19 + 1*viol.r_19

            t2 ~ t1
            t3 ~ t2

            m_rep =~ 1*viol.p_15 + 1*viol.p_17 + 1*viol.p_19
            m_vic =~ 1*viol.r_15 + 1*viol.r_17 + 1*viol.r_19
            m_loc =~ 1*viol.f_15 + 1*viol.f_17 + 1*viol.f_19
            ...
            viol.p_15 + viol.p_17 + viol.p_19 + viol.r_15 + viol.r_17 + viol.r_19 + viol.f_15 + viol.f_17 + viol.f_19 ~ 0*1

           ")
m2_viol_uw <- bsem(model, data = b2yw_uw,
                burnin = 8000, sample = 2000, n.chains = 8)

summary(m2_viol_uw, standardized = TRUE)


qs_m2_uw <- list(m2_propvh_uw, m2_proppers_uw, m2_viol_uw)

save(qs_m2_uw, file = "./output/qs_m2_uw.RData")



load("./output/qs_m2_uw.RData")
...
ggsave("./output/figs/qsm2_quality_uw.png")
...
ggsave("./output/figs/qsm2_mean_uw.png")
...
qs <- function(var) {
  ...
  bsem(model, data = b2yw_uw,
       burnin = 8000, sample = 2000, n.chains = 8)
}

res_qs_uw <- map(var_int3, qs)
save(res_qs_uw, file = "./output/quasi_simple_uw.RData")

load("./output/quasi_simple_uw.RData")


qs_rel_uw <- map(res_qs_uw, function(x) summary(x, standardized = TRUE) %>%
                .[1:3, 7] %>% as.numeric()
) %>%
  reduce(cbind) %>%
  as_tibble() %>%
...
qs_rel_uw %>%
  ...
  mutate(group = case_when(str_detect(key, "\\.r") ~ "Survey victim",
                           str_detect(key, "\\.p") ~ "Survey reported",
                           str_detect(key, "\\.f") ~ "Survey places",
                           TRUE ~ "Police"),
         var = str_remove_all(key, "\\.f|\\.p|\\.r"),
         topic = case_when(
           str_detect(key, "prop_pers") ~ "Property personal",
           str_detect(key, "prop_vh") ~ "Property household/vehicle",
           TRUE ~ "Violence")) %>%
...
b6yw2_wo <- barca_6y %>%
  rename_all(~str_remove_all(., "erty|onal|ence")) %>%
  select(all_of(vars_int2_wo), vars_int2[c(1, 5, 9)]) %>%
  mutate_all(~log(. + 1))
...
ggsave("./output/figs/mtmm_mean_overall_wo.png")
...
ggsave("./output/figs/qsm2_mean_wo.png")
